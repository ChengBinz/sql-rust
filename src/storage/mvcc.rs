...
#[cfg(test)]
mod tests {
    use crate::{error::Result, storage::{disk::DiskEngine, engine::Engine, memory::MemoryEngine}};
    ...
    fn get(eng: impl Engine) -> Result<()> {
        ...
        txn.set(b"key2".to_vec(), b"val3".to_vec())?;
        txn.set(b"key3".to_vec(), b"val4".to_vec())?;
        txn.delete(b"key3".to_vec())?;
        txn.commit()?;

        let txn1 = mvcc.begin()?;
        assert_eq!(txn1.get(b"key1".to_vec())?, Some(b"val1".to_vec()));
        assert_eq!(txn1.get(b"key2".to_vec())?, Some(b"val3".to_vec()));
        assert_eq!(txn1.get(b"key3".to_vec())?, None);
    ...
    fn get_isolation(eng: impl Engine) -> Result<()> {
        ...
        let txn1 = mvcc.begin()?;
        txn1.set(b"key1".to_vec(), b"val2".to_vec())?;

        let txn2 = mvcc.begin()?;

        let txn3 = mvcc.begin()?;
        txn3.set(b"key2".to_vec(), b"val4".to_vec())?;
        txn3.delete(b"key3".to_vec())?;
        txn3.commit()?;

        // assert_eq!(txn2.get(b"key1".to_vec())?, Some(b"val1".to_vec()));
        println!("{:?}", txn2.get(b"key1".to_vec())?);
        println!("{:?}", b"val1".to_vec());
        assert_eq!(txn2.get(b"key2".to_vec())?, Some(b"val3".to_vec()));
        assert_eq!(txn2.get(b"key3".to_vec())?, Some(b"val4".to_vec()));
    ...
    fn scan_prefix(eng: impl Engine) -> Result<()> {
        let mvcc = Mvcc::new(eng);
        let txn = mvcc.begin()?;
        txn.set(b"aabb".to_vec(), b"val1".to_vec())?;
        txn.set(b"abcc".to_vec(), b"val2".to_vec())?;
        txn.set(b"bbaa".to_vec(), b"val3".to_vec())?;
        txn.set(b"acca".to_vec(), b"val4".to_vec())?;
        txn.set(b"aaca".to_vec(), b"val5".to_vec())?;
        txn.set(b"bcca".to_vec(), b"val6".to_vec())?;
        txn.commit()?;

        let txn1 = mvcc.begin()?;
        let iter1 = txn1.scan_prefix(b"aa".to_vec())?;
        assert_eq!(
            iter1,
            vec![
                super::ScanResult {
        ...
        let iter2 = txn1.scan_prefix(b"a".to_vec())?;
        assert_eq!(
            iter2,
            vec![
                super::ScanResult {
        ...
        let iter3 = txn.scan_prefix(b"bcca".to_vec())?;
        assert_eq!(
            iter3,
