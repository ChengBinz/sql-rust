...
impl DiskEngine {
    ...
    pub fn new_compact(file_path: PathBuf) -> Result<Self> {
        let mut engine = Self::new(file_path)?;
        engine.compact()?;
        Ok(engine)
    }
    ...
    fn build_keydir(&mut self) -> Result<KeyDir> {
        ...
        loop {
            ...
            if val_size == -1 {
                ...
                offset += key_size as u64 + LOG_HEADER_SIZE as u64 + val_size as u64;
            }
        }
...
fn test_disk_engine_start() -> Result<()> {
    let engine = DiskEngine::new_compact(PathBuf::from("/tmp/sqldb-log"))?;
    Ok(())
}
